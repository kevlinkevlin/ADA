...
int main()
{
    ...
    for (int l = 0; l <= k; l++)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < m; j++)
            {
                if (dp[l][i][j] > dp[l][mxpos[l].first][mxpos[l].second])
                    mxpos[l] = {i, j}; // record the mxpos in l-th layer
            }
        }
    }
